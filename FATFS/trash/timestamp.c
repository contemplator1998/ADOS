...
uint16_t format_rtc_time(char * buf, uint16_t buf_size, rtc_time * time, TimeFormat fmt, TimePrecision prec)
{
  ...
  else if (fmt == TF_ASCII_UNIX || (fmt == TF_HUMAN_SHORT && time->time_ref == TIMEREF_ARBITRARY))
  {
    ...
    {
      uint32_t scale = 1000000000L;
      uint32_t seconds = time->seconds;
      buf[buf_ptr++] = DIG(seconds/scale);
      for (int i = 0; i < 9; i++) 
      {
        uint32_t modsec = seconds % scale; 
        scale /= 10;
  ...
  else if (fmt == TF_ASCII_EXCEL)
  {
    ...
    if (buf_size >= 16)
    {
      ...
      uint32_t days_since_1900 = (time->seconds / 86400) + 25569;
  
      // compute nanoseconds of day
      uint64_t nanosec_of_day = (((time->seconds % 86400) * 1000000ULL)
        + (uint64_t)(time->microsec)) * 1000ULL;
      
      // compute day fraction (in 1e-10 days)
      uint64_t day_frac = nanosec_of_day / 8640;
      ...
      uint64_t scale = 1000000000ULL;
      buf[buf_ptr++] = DIG(day_frac/scale);
      for (int i = 0; i < 9; i++) 
      {
        uint32_t modsec = day_frac % scale; 
        scale /= 10;
        buf[buf_ptr++] = DIG(modsec / scale);
      }
    }
...
/*---------------------------------------------------------------------------------
// implementation for FatFs support	   (перенесла в ff.c)
